...
if(file_exists('txt\todo.txt'))
{
	//membaca file todo.txt
	$file	=	file_get_contents('txt\todo.txt');
...
{
    file_put_contents('txt\todo.txt',$daftar_belanja);
	header('location:4_AplikasiTodoList.php');
}
// print_r($todos);
?>
<h1>Todo App</h1>
...
<ul>
    ...
    <li>
        <input type="checkbox" name="todo" onclick="window.location.href='4_AplikasiTodoList.php?status=<?php echo ($value['status'] == 1) ? '0' : '1'; ?>&key=<?php echo $key; ?>'" <?php if ($value['status'] == 1) echo 'checked'; ?>>
        <label>
            <?php
            if ($value['status'] == 1) {
        ...
        <a href="4_AplikasiTodoList.php?hapus=1&key=<?php echo $key; ?>" onclick="return confirm('Apakah Anda Yakin akan menghapus data ini?')">hapus</a>
    </li>
    <?php endforeach; ?>
</ul>